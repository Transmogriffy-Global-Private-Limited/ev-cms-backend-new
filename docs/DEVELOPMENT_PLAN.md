...
- Source implementation adds the distinct CUSTOMER_CPO durable support channel
  ...
  binds CUSTOMER message/event actors to the ticket's exact customer owner; and
  migration 74 prevents a ticket-owner change from cascading into that immutable
  historical actor identity. Source checks are recorded in the archived support,
  actor-ownership, and owner-immutability work items; PostgreSQL-gated tests
  were skipped because no disposable `TEST_DATABASE_URL` was selected.
  Deployment and live migration are not part of this plan entry.
...
- The repaired CPO charging-session list contract is deployed in CMS revision
  `6d9d15a` without a new migration. Generic and legacy pagination, sorting,
...
- The earlier emergency correction source checkpoint for the deployed `2ad082f` CPO
  ...
  `TEST_DATABASE_URL` is selected. The repair is deployed as recorded above;
...
- Deployed reconciliation checkpoint: an open materialized CMS session can
  ...
  `3ea7b76` on top of `ced8b65`. Source migration `000067` gives the existing
  worker a durable circular candidate cursor so bounded active/404/transport
  observations cannot starve later sessions. Invalid/mismatched successful
  reads and contradictory terminal evidence retain occupancy as
...
- CPO `chargers.operations` live-session operations: `GET /operations/live-sessions` is the
  ...
  deployed runtime combines canonical normal-session static context (nested
