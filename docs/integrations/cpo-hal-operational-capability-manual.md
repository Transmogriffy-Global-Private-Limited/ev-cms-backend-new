...
| `transaction.completed` | `hal_transaction_id`, `ocpp_transaction_id`, `cms_start_intent_id`, `charger_ocpp_identity`, `ocpp_connector_number`, `meter_start_wh`, `meter_stop_wh`, `stopped_at`, `requested_stop_initiator`, `requested_stop_reason`, `ocpp_stop_reason` | validates final fact then completes/settles exactly once while preserving requested-stop provenance separately from charger `StopTransaction.reason`; local stop is valid without a CMS stop command. |
