...
overwrite the replacement. It is deployed in revision `040b9bb`; migrations 39
is applied and the service has one current row per logical worker.

The current shared development deployment is runtime source revision `040b9bb`
with migrations through forty-one and 180 OpenAPI operations. The HAL runtime
