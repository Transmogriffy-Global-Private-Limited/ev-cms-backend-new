...
func RegisterRoutes(group *gin.RouterGroup, authService *auth.Service, service *Service) {
	...
	group.POST("/cpos/:cpo_id/subscription", handler.assign)
...
func (handler *Handler) assign(ctx *gin.Context) {
	...
	response, err := handler.service.Assign(ctx.Request.Context(), principal, cpoID, request)
	write(ctx, http.StatusCreated, response, err)
}
...
func (handler *Handler) pause(ctx *gin.Context) {
	handler.transition(ctx, handler.service.Pause)
}

func (handler *Handler) resume(ctx *gin.Context) {
	handler.transition(ctx, handler.service.Resume)
}

func (handler *Handler) cancel(ctx *gin.Context) {
	handler.transition(ctx, handler.service.Cancel)
}
...
func (handler *Handler) expire(ctx *gin.Context) {
	handler.transition(ctx, handler.service.Expire)
}
...
func (handler *Handler) deleteOverride(ctx *gin.Context) {
	...
	err := handler.service.DeleteOverride(ctx.Request.Context(), principal, cpoID, ctx.Param("feature_key"), reason)
	if err != nil {
		writeError(ctx, err)
		return
	}
