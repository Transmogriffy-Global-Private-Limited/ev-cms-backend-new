...
type Tariff struct {
	...
	Hub   Hub       `gorm:"foreignKey:HubID;constraint:OnUpdate:CASCADE,OnDelete:RESTRICT" json:"hub,omitempty"`
	// AssignedTo mirrors migration 34's nullable PostgreSQL enum column. The
	// current tariff APIs do not yet assign this classification, so existing and
	// newly created rows must remain valid with a NULL value.
	AssignedTo    *constants.TariffAssignmentType `gorm:"column:assigned_to;type:tariff_assignment_type" json:"assigned_to,omitempty"`
